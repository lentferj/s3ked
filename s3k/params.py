...
_PARAMS: List[Parameter] = [
    ...
    _p(
        ...
        notes="range as written: \"-50.00 to +50.00 (fraction is binary)\" -- those are SEMITONES, and the raw unit is 1/256 of one. Declared 0..50 here until 2026-08-12, which was the document's display range transcribed as a value range and 256x too narrow: it makes 19.53 cents the largest detune the field can express. The settable grid is ONE CENT, not 1/256 of a semitone: 2.56 raw units per cent, and a value off the grid is silently snapped to the nearest on it (raw 5121 stores as 5120). Compute raw from cents as trunc(cents * 2.56) and read back if it matters. The panel shows two decimals of a semitone, which is exactly the grid, so display and byte agree at every reachable value -- §89. Measured by writing raw values and reading the PITCH back -- 256 gave +99.8 cents, 512 +199.8, 1280 +499.9, 2560 +999.9, 5120 +1999.9, and the negatives match to 0.3 cents. Every value round-tripped exactly, negatives as two's complement. Beyond +-5120 (+-20 semitones) the scale is unverified: the pitch detector tops out, and whether the sampler itself will transpose that far is untested -- and mpc2emu has since seen zones carrying 7680 and 11520 (+30 and +45 semitones) play SILENTLY while everything else about them was identical. A high tune can mute a zone rather than being clamped, so a value beyond +-5120 is not merely unverified but possibly inaudible. The boundary is unmeasured. RESOLUTION_NOTES §56.",
    ),
    _p(
        "program",
        67,
    ...
    _p("multi", 16, "FX1", 1, "multi.effects", 0, 204, models="S2000/S3000XL/S3200XL",
        ...
        notes="measured on hardware 2026-09-10 (§215): the device REFUSES 205-238 and accepts 0-204. The spec gives no range, so 0..255 was the byte. The declared maximum is 204 because writing 239 crashed the machine to 'Internal Error - divide overflow' and read back as 205 -- the refused band is 34 wide and 239-34 = 205, so the high tail appears to fold into the invalid region. Values above 204 must not be offered. PANEL READ 2026-09-10 (§216): the EFFECTS/REVERB SELECT page shows 38 CLEAR DETUNE for byte 37 and 1 REVERB EQ 1 for byte 0, so the panel is 1-based and the byte is 0-based -- the POLYPH convention, confirmed twice inside one photograph"),
